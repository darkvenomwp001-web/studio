    ...
    return (
        <Card className="w-full max-w-md shadow-2xl bg-card/80 backdrop-blur-sm border-border/20">
            ...
            <CardContent className="space-y-4">
                 <Button onClick={handleCheckVerification} className="w-full" disabled={isChecking || authLoading}>
                    {isChecking ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                    I've Verified My Email
                </Button>
                <Button onClick={handleResendEmail} variant="secondary" className="w-full" disabled={isSending || authLoading}>
