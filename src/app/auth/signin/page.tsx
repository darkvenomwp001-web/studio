      ...
      <Card className="w-full max-w-sm shadow-2xl bg-card/80 backdrop-blur-sm border-border/20">
        <CardHeader className="text-center space-y-1">
          <CardTitle className="text-3xl font-headline text-primary">
            {hint ? `Welcome back` : `Sign In`}
          </CardTitle>
          <CardDescription>
            {hint ? `Ready to resume your session as @${hint}?` : isAddingAccount ? `Add an account to your switcher.` : `Sign in to your DVHIDEOUT account.`}
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-6">
            ...
            <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground text-lg py-6 shadow-lg shadow-primary/30" disabled={isAnyLoading}>
              {authLoading && !initialAuthLoading && (emailOrUsername || password) ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <><LogIn className="mr-2 h-5 w-5" /> {hint ? 'Resume Session' : 'Sign In'}</>}
            </Button>
